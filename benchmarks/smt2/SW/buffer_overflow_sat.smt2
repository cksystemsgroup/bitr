...
; Software verification: buffer overflow — reachable case.
; Array buf[256] of bytes indexed by 8-bit address.
; Write value 0x42 at index i where i < 16 (unsigned).
; Original buf has 0x00 at index 10.
; Assert reading index 10 from modified array yields 0x42.
; SAT: i can be 10, which is in {0..15}.

(declare-fun buf () (Array (_ BitVec 8) (_ BitVec 8)))
(declare-const i (_ BitVec 8))

; Bound: i < 16
(assert (bvult i (_ bv16 8)))
...
; Claim: reading index 10 from buf1 yields 0x42
